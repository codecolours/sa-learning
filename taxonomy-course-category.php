 ...
 */
$container_class = apply_filters( 'neve_container_class_filter', 'container', 'course-category-archive' );

get_header();

$wrapper_classes = [ 'posts-wrapper', 'course-grid', 'items-wrapper', 'flex-container' ];
...
?>
<div class="<?php echo esc_attr( $container_class ); ?> archive-container course-category-archive">
	 ...
	 * @since 3.2
	 */
	do_action( 'neve_do_featured_post', 'course-category' );
	 ...
	 */
	do_action( 'neve_page_header', 'course-category' );

	// Display category title and description
	$current_term = get_queried_object();
	if ( $current_term && ! empty( $current_term->name ) ) :
		?>
		<div class="category-header smt-large">
			<h1 class="course-category-title"><?php echo esc_html( $current_term->name ); ?></h1>
			<?php if ( ! empty( $current_term->description ) ) : ?>
				<div class="course-category-description">
	...
	<div class="course-category-content">
		<div class="nv-index-posts course-listing-block">
			<?php
			/**
			...
			if ( have_posts() ) {
				...
				while ( have_posts() ) {
					...
					// Use custom course item template
				...
				if ( ! is_singular() ) {
					 ...
					 */
					do_action( 'neve_do_pagination', 'course-category-archive' );
				}
			} else {
