<?php
/**
 * Custom Blocks for the website.
 *
 * @package sa-learning
*/

// Check whether WordPress and ACF are available
...
add_filter('site-acf-blocks-templates', function () {
    return ['views/blocks'];
});
...
add_action('acf/init', function () {
    ...
    // Global $block_error so we can throw errors.
    global $block_error;

    // Get an array of directories containing blocks
    $directories = apply_filters('site-acf-blocks-templates', []);
        ...
        foreach ($template_directory as $template) {
            if (!$template->isDot() && !$template->isDir()) {
                ...
                if (empty($file_headers['title'])) {
                    $block_error(__('This block needs a title: ' . $dir . '/' . $template->getFilename(), 'sa-learning'), __('Block title missing', 'sa-learning'));
                }

                if (empty($file_headers['category'])) {
                    $block_error(__('This block needs a category: ' . $dir . '/' . $template->getFilename(), 'sa-learning'), __('Block category missing', 'sa-learning'));
                }
                ...
                $data = [
                    ...
                    'render_callback'   => 'site_blocks_callback',
                ...
                acf_register_block(apply_filters("site/blocks/$slug/register-data", $data));
 ...
 */
function site_blocks_callback($block, $content = '', $is_preview = false, $post_id = 0)
{
    ...
    $block = apply_filters("site/blocks/$slug/data", $block);
    ...
    foreach ($directories as $directory) {
        ...
        //echo \locate_template("$directory/$slug.php");
        $template_path = locate_template("$directory/$slug.php");
        echo template($template_path, ['block' => $block]);
    }
}
...
function template($template, $data = []) {
    ...
    return ''; // Return empty string if the template doesn't exist
}